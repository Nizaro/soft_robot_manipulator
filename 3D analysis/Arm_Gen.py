...
def EndConstruct(P,Q,points,m,ray,L):
    Ex=np.array([1,0,0])
    Ey=np.array([0,1,0])
...
def SymPHrandom(N,L,rep):
    ...
    r=np.empty([N])
    for i in range(N//rep):
        theta[rep*i]=random.random()*2*np.pi/4
        phi[rep*i]=random.random()*2*np.pi
...
rPCC=np.array([L*np.sin(theta)/theta])
rSPH=np.array([L*((2*np.cos(theta)+1)/(np.cos(theta)+2))])

PCCpoints,PCCQ=General_Construct(phi, theta, rPCC)
SPHpoints,SPHQ=General_Construct(phi, theta, rSPH)
PCCcurve,QnQ=MultiPCC3D(PCCpoints, PCCQ, 101)
SPHcurve,QNS=MultiPH3D(np.transpose(SPHpoints), L)
Straight=np.array([[0,0,0],[0,0,1]])

color=['b','g','r','c','m','y','k']
'''
'''
ax = plt.axes(projection='3d')
ax.plot3D(SPHcurve[:,0,0],SPHcurve[:,1,0],SPHcurve[:,2,0],label='SPH')
ax.plot3D(PCCcurve[:,0,0],PCCcurve[:,1,0],PCCcurve[:,2,0],label='PCC')
ax.plot3D(Straight[:,0],Straight[:,1],Straight[:,2],label='PCC')
ax.set_xlabel('x')
ax.set_ylabel('y')
ax.set_zlabel('z')
ax.legend(loc='upper left')
...
plt.plot(SPHcurve[:,0,0],SPHcurve[:,2,0],label='SPH')
plt.plot(PCCcurve[:,0,0],PCCcurve[:,2,0],label='PCC')
plt.plot(Straight[:,0],Straight[:,2],label='Repos')
plt.legend()
'''